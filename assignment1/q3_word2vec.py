...
def normalizeRows(x):
    ...
    ### YOUR CODE HERE
    x = x / np.sqrt(np.sum(x * x, axis=1))[:, np.newaxis]
    ### END YOUR CODE
...
def softmaxCostAndGradient(predicted, target, outputVectors, dataset):
    ...
    gradPred = np.sum(outputVectors * yhat[:, np.newaxis], axis=0) \
...
def negSamplingCostAndGradient(predicted, target, outputVectors, dataset,
    ...
    cost = -np.log(sigmoid(np.dot(outputVectors[target], predicted))) \
            - sum(np.log(sigmoid(-np.dot(outputVectors[k], predicted))) for k in indices[1:])

    gradPred = (sigmoid(np.dot(outputVectors[target], predicted)) - 1.0) * outputVectors[target] \
            + sum((1.0 - sigmoid(-np.dot(outputVectors[k], predicted))) * outputVectors[k] for k in indices[1:])
